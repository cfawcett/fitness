...
func (h *Handler) registerRoutes(auth *authenticator.Authenticator) {
	...
	// --- Superset Routes (New) ---
	h.Router.POST("/activity/:id/create-superset", middleware.IsAuthenticated, workout.CreateOrAddToSupersetHandler(h.ExerciseRepo, h.GymExerciseRepo))
	h.Router.POST("/gym-exercise/:id/remove-from-superset", middleware.IsAuthenticated, workout.RemoveFromSupersetHandler(h.ActivityRepo, h.ExerciseRepo, h.GymExerciseRepo))
}
